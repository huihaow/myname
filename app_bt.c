...
void bt_create_conn_timeout_wrap(void *arg)
{
    ...
    if(bt_flag1_is_set(APP_FLAG_AUTO_CONNECTION))
    {
		...
		if(exception_restart_flag)
			max_count = 0x0A;                   //reconnect count 10 times
		#endif
...
void bt_auto_connect_stop(void)
{
    ...
    os_printf("bt_auto_connect_stop\r\n");

    jtask_stop(sys_hdl->app_auto_con_task);
    retry_count = 0;
#ifdef APP_ENV_RESTART_MUSIC_RECOVER
    exception_restart_flag = 0;
#endif
...
//        app_led_action(1);
    ...
    else if(sys_hdl->flag_sm1 & APP_FLAG_LINEIN)
    {
        jtask_stop(sys_hdl->app_auto_con_task);
...
*/
#if(CONFIG_CUSTOMER_SET_SUPERVISION_TIME == 1)
	memcpy( (uint8 *)&sys_hdl->stereo_btaddr, (uint8 *)rbtaddr,
		sizeof(btaddr_t) );
...
void app_bt_disconn_wrap(uint8 reason, btaddr_t *raddr)
{
    ...
    os_printf("bt_disconn_reason:0x%x, f_sm1:0x%x, raddr:"BTADDR_FORMAT"\r\n", reason,sys_hdl->flag_sm1,BTADDR(raddr));
...
//#endif
    ...
    sys_hdl->flag_sm1 |= pflag;
			...
			! bt_flag2_is_set( APP_FLAG2_STEREO_ROLE_MASTER) )
		{
            ...
            #ifdef CONFIG_CRTL_POWER_IN_BT_SNIFF_MODE
                bt_flag1_operate(APP_FLAG_MODE_CHANGE_SETTING|APP_FLAG_SNIFF_MODE_CHANGE,0);
            #endif
